"""
Topic Processing Layer - Improved Local LLM Version
Implements intelligent topic detection and grouping with better source aggregation
"""
...
from local_llm_service_improved import local_llm

class TopicProcessor:
    """Processes articles into topics using improved local LLM intelligence"""
    ...
    async def process_articles(self, articles: List) -> List[Dict[str, Any]]:
        ...
        print(f"🧠 Processing {len(articles)} articles into topics using improved local LLM...")
    ...
    async def _create_topic_from_group(self, articles: List) -> Dict[str, Any]:
        """Create a topic from a group of articles using improved local LLM"""
        if not articles:
            return None
        ...
        try:
            ...
            # Generate title and summary using improved local LLM
            title = await local_llm.generate_topic_title(articles_text)
            summary = await local_llm.generate_summary(articles_text)
            ...
            # Create topic with improved structure
            topic = {
