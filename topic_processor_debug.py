"""
Debug Topic Processor
Fixes over-grouping and restores proper source diversity
"""
...
class TopicProcessor:
    """Processes articles into topics with proper grouping"""
    ...
    async def process_articles(self, articles: List) -> List[Dict[str, Any]]:
        ...
        try:
            # Use LESS aggressive grouping to preserve diversity
            grouped_articles = self._conservative_grouping(articles)
    ...
    def _conservative_grouping(self, articles: List) -> List[List]:
        """Conservative grouping that preserves more topics"""
        groups = []
        used_articles = set()
        ...
        for i, article in enumerate(sorted_articles):
            ...
            # Find similar articles with STRICTER matching
            for j, other_article in enumerate(sorted_articles[i+1:], i+1):
                if j in used_articles:
                    continue
                    
                if self._are_articles_strongly_related(article, other_article):
    ...
    def _are_articles_strongly_related(self, article1, article2) -> bool:
        """STRICTER similarity check to prevent over-grouping"""
        ...
        words1 = set([w for w in title1.split() if w not in stop_words and len(w) > 3])
        words2 = set([w for w in title2.split() if w not in stop_words and len(w) > 3])
        ...
        # MUCH stricter grouping - require high similarity
        return similarity >= 0.6 or len(common_words) >= 4
    
    async def _create_topic_from_group(self, articles: List) -> Dict[str, Any]:
        """Create a topic from a group of articles"""
        if not articles:
            return None
