...
<head>
    ...
    <style>
        ...
        /* Responsive grid for feed cards */
        .feed-grid {
          display: grid;
        ...
        /* Ensure cards are left-aligned */
        .article-card, .cluster-card { text-align: left; }
        .article-card h3, .cluster-header h3 { margin: 0 0 8px 0; line-height: 1.25; }
        .excerpt, .cluster-excerpt { text-align: left; }
        .article-meta { gap: 12px; row-gap: 8px; }
        @media (max-width: 360px) { .article-card, .cluster-card { padding: 16px; } }
    ...
    function formatDate(s) {
        ...
        async function loadFeed() {
            try {
                ...
                // Wrap in responsive grid (1 col mobile, 2 cols >=700px)
...
def start_processor():
    ...
    try:
        ...
        def run_processor():
            ...
            asyncio.run(processor.run_continuous_processor(max_articles=10, delay_between_articles=5))
